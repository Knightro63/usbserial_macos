...
open class SerialDeviceMonitor {
    var serialDevices:[SerialDeviceInfo] = []
    var portUsageIntervalTime:Float = 0.25
    open var filterDevices:((_ devices:[SerialDeviceInfo])->[SerialDeviceInfo])?
    ...
    func getSerialDevices(iterator: io_iterator_t) {
        var newSerialDevices:[SerialDeviceInfo] = []
        while case let serialPort = IOIteratorNext(iterator), serialPort != 0 {
            ...
            var sd = SerialDeviceInfo(path: calloutDevice)
